...
To install Magento via Composer, you need to register an account to get the Authentication Keys from [Magento Markplace](https://marketplace.magento.com/). In step 2 next, tou will use the public key to assign to SSH variable `magentouser_key`and private key to assign to SSH variable `magentouser_pass`.
...
### Step 3. Composer install Magento

Below steps will install Magento 2.2.2 via Composer installation on Centmin Mod LEMP stack based CentOS 7.4 64bit server enabling the following optimisations:
